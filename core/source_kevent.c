...
_dispatch_get_next_timer_fire(struct timespec *howsoon)
{
    ...
    struct dispatch_source_s* ds = NULL;
    unsigned int timer;
...
static size_t
_dispatch_source_kevent_debug(struct dispatch_source_s* ds, char* buf, size_t bufsiz)
{
...
static void _dispatch_source_merge_kevent(struct dispatch_source_s* ds, const struct kevent *ke);
...
static void
_dispatch_kevent_debugger2(void *context, struct dispatch_source_s* unused DISPATCH_UNUSED)
{
    struct sockaddr sa;
...
void
_dispatch_kevent_release(struct dispatch_source_s* ds)
{
    ...
    _dispatch_release((struct dispatch_object_s*)ds);	// the retain is done at creation time
}

void
_dispatch_source_merge_kevent(struct dispatch_source_s* ds, const struct kevent *ke)
{
    struct kevent fake;
...
void
dispatch_source_merge_data(dispatch_source_t obj, unsigned long val)
{
...
static bool
dispatch_source_type_kevent_init(struct dispatch_source_s* ds, dispatch_source_type_t type, uintptr_t handle, unsigned long mask, dispatch_queue_t q)
{
...
static bool
dispatch_source_type_timer_init(struct dispatch_source_s* ds, dispatch_source_type_t type, uintptr_t handle, unsigned long mask, dispatch_queue_t q)
{
...
static bool
dispatch_source_type_mach_send_init(struct dispatch_source_s* ds, dispatch_source_type_t type, uintptr_t handle, unsigned long mask, dispatch_queue_t q)
{
    static dispatch_once_t pred;
...
static bool
dispatch_source_type_mach_recv_init(struct dispatch_source_s* ds, dispatch_source_type_t type, uintptr_t handle, unsigned long mask, dispatch_queue_t q)
{
    if (!dispatch_source_type_kevent_init(ds, type, handle, mask, q)) {
        return false;
    }
...
void
_dispatch_timer_list_update(struct dispatch_source_s* ds)
{
    struct dispatch_source_s* dsi = NULL;
    int idx;
