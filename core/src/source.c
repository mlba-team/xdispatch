...
static void _dispatch_source_latch_and_call(struct dispatch_source_s*  ds);
static void _dispatch_source_cancel_callout(struct dispatch_source_s* ds);
static size_t dispatch_source_debug_attr(struct dispatch_source_s* ds, char* buf, size_t bufsiz);
...
DISPATCH_NOINLINE
void
_dispatch_source_xref_release(struct dispatch_source_s* ds)
{
#ifndef DISPATCH_NO_LEGACY
    if (ds->ds_is_legacy) {
    ...
    _dispatch_wakeup((struct dispatch_queue_s*)ds);
    _dispatch_release((struct dispatch_object_s*)ds);
}

long
dispatch_source_testcancel(dispatch_source_t ds)
{
    return (bool)(DSOURCE_CAST(ds)->ds_atomic_flags & DSF_CANCELED);
}
...
{
    return DSOURCE_CAST(ds)->ds_pending_data_mask;
}

uintptr_t
dispatch_source_get_handle(dispatch_source_t ds)
{
    return (int)DSOURCE_CAST(ds)->ds_ident_hack;
}

unsigned long
dispatch_source_get_data(dispatch_source_t ds)
{
    return DSOURCE_CAST(ds)->ds_data;
}

struct dispatch_queue_s*
_dispatch_source_invoke(struct dispatch_source_s* ds)
{
    ...
    struct dispatch_queue_s* dq = _dispatch_queue_get_current();
...
bool
_dispatch_source_probe(struct dispatch_source_s* ds)
{
...
void
_dispatch_source_dispose(struct dispatch_source_s* ds)
{
    _dispatch_queue_dispose((struct dispatch_queue_s*)ds);
}

void
_dispatch_source_latch_and_call(struct dispatch_source_s* ds)
{
    unsigned long prev;
...
void
_dispatch_source_cancel_callout(struct dispatch_source_s*  ds)
{
...
size_t
dispatch_source_debug_attr(struct dispatch_source_s* ds, char* buf, size_t bufsiz)
{
    struct dispatch_queue_s* target = ds->do_targetq;
...
size_t
_dispatch_source_debug(struct dispatch_source_s* ds, char* buf, size_t bufsiz)
{
        size_t offset = 0;
        offset += snprintf(&buf[offset], bufsiz - offset, "%s[%p] = { ", dx_kind(ds), ds);
        offset += dispatch_object_debug_attr((struct dispatch_object_s*)ds, &buf[offset], bufsiz - offset);
        offset += dispatch_source_debug_attr((struct dispatch_source_s*)ds, &buf[offset], bufsiz - offset);
        return offset;
}
...
_dispatch_source_set_event_handler2(void *context)
{
    ...
    struct dispatch_source_s* ds = (struct dispatch_source_s*)_dispatch_queue_get_current();
