...
#ifndef __DISPATCH_QUEUE_INCLUDED
# include "internal.h"
#endif

#ifdef __DISPATCH_QUEUE_INCLUDED
...
_dispatch_continuation_pop(dispatch_object_t dou)
{
    ...
    if (DISPATCH_OBJ_IS_VTABLE(DO_CAST(dou))) {
        return _dispatch_queue_invoke(DQUEUE_CAST(dou));
    }
...
dispatch_queue_debug(dispatch_queue_t dq, char* buf, size_t bufsiz)
{
    ...
    offset += dispatch_object_debug_attr(DO_CAST(dq), &buf[offset], bufsiz - offset);
